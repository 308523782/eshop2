...
public class OrderController extends BaseController {
	...
	public @ResponseBody
	Map<String, Object> calculate(Orders order, Long areaId, Long paymentMethodId, Long shippingMethodId) {
		Map<String, Object> data = new HashMap<String, Object>();
		...
		Orders pOrder = orderService.find(order.getId());
		if (pOrder == null) {
	...
	public String update(Orders order, Long areaId, Long paymentMethodId, Long shippingMethodId, RedirectAttributes redirectAttributes) {
		for (Iterator<OrderItem> iterator = order.getOrderItems().iterator(); iterator.hasNext();) {
			OrderItem orderItem = iterator.next();
			if (orderItem == null || StringUtils.isEmpty(orderItem.getSn())) {
				iterator.remove();
