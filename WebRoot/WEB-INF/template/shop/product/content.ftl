...
$().ready(function() {
	...
	for (var i = 0; i < historyGoodsIds.length; i ++) {
		if (historyGoodsIds[i] == 3) {
			historyGoodsIds.splice(i, 1);
			break;
		}
	}
	historyGoodsIds.unshift(3);
		...
		<div class= "row">
			<div class="span2">
				...
				<div id="historyGoods" class="historyGoods">
					<dl>
						<dt>${message("shop.product.historyProduct")}</dt>
						<dd>
						</dd>
						<dd>
							<a href="javascript:;"  class="clear">${message("shop.product.clearHistoryProduct")}</a>
						</dd>
					</dl>
				</div>
			</div>
			<div class="span10">
				...
				<div class="info">
					<h1>
						${product.name}[#if product.isGift] [${message("shop.product.gifts")}][/#if]
						<em></em>
					</h1>
					<dl>
